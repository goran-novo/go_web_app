...
sudo -i -u postgres psql -c "CREATE DATABASE green"
sudo -i -u postgres psql -d green -c "CREATE EXTENSION IF NOT EXISTS citext"
sudo -i -u postgres psql -d admin -c "CREATE ROLE admin WITH LOGIN PASSWORD '${DB_PASSWORD}'"

# Add a DSN for connecting to the greenlight database to the system-wide environment 
# variables in the /etc/environment file.
echo "GREEN_DB_DSN='postgres://green:${DB_PASSWORD}@localhost/admin'" >> /etc/environment
